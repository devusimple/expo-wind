...
CSS variables are natively supported in React Native via Tailwind CSS v4. By defining `--color-primary`, `--color-background`, etc. in `global.css` using `@layer theme` with `@variant light` / `@variant dark`, every component automatically responds to theme changes without:

- Re-rendering the entire tree
- Passing a theme object through context
- Re-computing styles in JavaScript

Theme switching is driven by `@variant dark` (Tailwind CSS v4's equivalent of `@media (prefers-color-scheme: dark)`) — no toggle needed. The CLI provides 8 preset themes selected during `init`.
...
Themes are stored as CSS in the registry and served during `init`. Each theme defines `@layer theme` blocks with `@variant light` and `@variant dark` sections. The CLI writes the selected theme's CSS to `global.css`. The theme name is also saved to `components.json` for reference.

Currently 8 preset themes are available: Neutral, Blue, Amber, Cyan, Emerald, Fuchsia, Green, and Default (Minimal).
