...
Components reference CSS variables via Tailwind utility classes (e.g., `bg-background`, `text-foreground`, `border-border`). Variables are defined in `global.css` using the `--color-*` prefix convention within `@layer theme` blocks.
...
- [ ] Reference CSS variables via Tailwind utility classes (`bg-background`, `text-foreground`, etc.)
